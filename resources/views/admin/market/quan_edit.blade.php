...
<div class="page-container">
    <form  method="get" name="form1"   action="{{url('/admin/balance/log_search')}}" >
        <div class="cl pd-5  mt-20 " >
            <input type="hidden"  id="quan_id" value="{{$quanDetail->id}}" >
            <p  class="distrib-open" style="width: 100%;  padding-bottom: 5px; border-bottom: 1px dashed #ddd;">
                ...
                <input  id="name"  type="text" class="input-mi"  style="width: 200px;"  value="{{$quanDetail->name}}" >
            </p>
            <p  class="distrib-open" style="width: 100%;  padding-bottom: 5px; border-bottom: 1px dashed #ddd;">
                <sapn style="display: inline-block; width:150px;  text-align: right;">面值：</sapn>
                <input  id="parvalue"  type="text" class="input-mi"  value="{{$quanDetail->parvalue}}"  onchange="onlyNum(this)" > &nbsp;元
            </p>
            <p  class="distrib-open" style="width: 100%;  padding-bottom: 5px; border-bottom: 1px dashed #ddd;">
                <sapn style="display: inline-block; width:150px;  text-align: right;">兑换所需积分：</sapn>
                <input  id="needpoint"  type="text" class="input-mi"   onchange="onlyNum(this)" value="{{$quanDetail->needpoint}}" > 个积分
            </p>
            <p style="width: 100%;  padding-bottom: 5px; border-bottom: 1px dashed #ddd;">
                ...
                满
                <input  tyep="text"  id="amount_reached"  name="amount_reached"   class="input-mi"  onchange="onlyNum(this)" value="{{$quanDetail->amount_reached}}"  >&nbsp;
                <label >  元使用</label>
            </p>
            <p style="width: 100%; padding-bottom: 5px; border-bottom: 1px dashed #ddd;">
                <sapn style="display: inline-block; width:150px;  text-align: right;">发放方式：</sapn>
                <input type="radio" id="get_type" name="get_type"    @if($quanDetail->get_type == 0)checked @endif value="0"><label for="enable">买家领取</label>

                <input type="radio" id="get_type" name="get_type"   @if($quanDetail->get_type == 1)checked @endif    value="1"><label for="enable">后台发放</label>
            </p>

            <p style="width: 100%;  padding-bottom: 5px; border-bottom: 1px dashed #ddd;">
                <sapn style="display: inline-block; width:150px;  text-align: right;">有效时间：</sapn>
                <input type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" id="logmax" class="input-text Wdate"    name="start_time"  value="{{ date('Y-m-d H:i:s',$quanDetail->start_time) }}"      style="width:180px;"  autocomplete="off" >
                -
                <input type="text"   name="end_time"onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss' })" id="logmin" class="input-text Wdate"   value="{{ date('Y-m-d H:i:s',$quanDetail->end_time) }}"               style="width:180px;"   autocomplete="off" >
            </p>
            <p style="width: 100%;  padding-bottom: 5px; border-bottom: 1px dashed #ddd;">
                <sapn style="display: inline-block; width:150px;  text-align: right;">限领：</sapn>
                <label >每人限领  &nbsp;</label>
                <input  tyep="text"  id="user_limit_num" class="input-mi"  onchange="onlyNum(this)"value="{{$quanDetail->user_limit_num}}"  >&nbsp;&nbsp;张，
                <label >每人每天限领  &nbsp;</label>
                <input  tyep="text"  id="user_day_num" class="input-mi"  onchange="onlyNum(this)" value="{{$quanDetail->user_day_num}}" >&nbsp;&nbsp;张
            </p>
            <p style="width: 100%;  padding-bottom: 5px; border-bottom: 1px dashed #ddd;">
            ...
            <p style="width: 100%;  padding-bottom: 5px; border-bottom: 1px dashed #ddd;">
                ...
                <input type="radio" id="is_enable" name="is_enable"   @if($quanDetail->is_enable == 1)checked @endif   value="0"><label for="enable">开启</label>
                <input type="radio" id="is_enable" name="is_enable" @if($quanDetail->is_enable ==0)checked @endif  value="1"><label for="enable">停用</label>
            </p>
...
@section('my-js')
    ...
    <script type="text/javascript">
        ...
        $('#sub').click(function(){
            ...
            var id =  parseInt($.trim($('#quan_id').val()) ) ; //面值
