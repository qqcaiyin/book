...
@section('content')
	...
	<div class="page-container">
        ...
        <form class="form form-horizontal" id="form-type-add" name="form-type-add"  onsubmit=" return check();" method="post">
            {{csrf_field()}}
            <div style="margin-left: 12px;" >
                <label >类型名称：</label>
                <div style="display:inline;"  >
                    <input type="text" class="input-text"  autocomplete="off"   style="width: 500px;"  placeholder="" name="type_name" datatype="*" nullmsg="名称不能为空"  value="{{$pdt_type->name}}">
        ...
        <span class="l">
            ...
            <div class="mt-20">
                <table class="table table-border table-hover table-bg table-sort table1">
                    <thead>
                    <tr class="text-c">
                        <th width="20%"  style="text-align:left;" >属性名</th>
                        <th width="20%">展示类型</th>
                        <th width="50%">选择项数据【每项数据之间用逗号','做分割】</th>
                        <th width="10%">是否规格</th>
                        <th width="10%">操作</th>
                    </tr>
                    </thead>
                    ...
                    <tr class="text-c trow" >
                        <td >
                            <input type="text" class="input-text"  autocomplete="off"   style="width: 200px;"  placeholder="" name="attr_name[{{$pdt_attr->id}}]" id="atte_name"  value="{{$pdt_attr->name}}">
                        </td>
                        <td >
                            <select class="select" name="uitype[{{$pdt_attr->id}}]" size="1" style="width:150px; height:30px;">
                            <option value="0" @if($pdt_attr->index == 0)  selected @endif >单选框 </option>
                            <option value="1"  @if($pdt_attr->index == 1)  selected @endif>复选框 </option>
                            <option value="2"  @if($pdt_attr->index == 2)  selected @endif> 输入框 </option>
                            </select>
                        </td>
                        <td>
                            <input type="text" class="input-text"  autocomplete="off"   style="width: 600px;"  placeholder="" name="vallist[{{$pdt_attr->id}}]"  value="{{$pdt_attr->value}}">
                        </td>
                        <td class="td-status">
                            @if($pdt_attr->is_guige == 1)
                                <a  href="javascript:;"  class="label label-success radius guige" >是</a>
                                <input type="hidden" name="is_guige[{{$pdt_attr->id}}]" value="1">
                            @else
                                <a  href="javascript:;"  class="label label-defaunt radius guige" >否</a>
                                <input type="hidden" name="is_guige[{{$pdt_attr->id}}]" value="0">
                            @endif
                        </td>
        ...
        $(function(){
            $("#form-type-add").validate({
                ...
                submitHandler:function(form){
                    $(form).ajaxSubmit({
                        type: 'post',
                        url: '/admin/service/type/edit', // 需要提交的 url
                        dataType: 'json',
                        data: {
                            id: '{{$pdt_type->id}}',
