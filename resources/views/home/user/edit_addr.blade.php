...
<body>
    ...
    <article class="add-address-center">
        <form method="post" action="{{url('/service/user')}} ">
            <input type="hidden"  name="id" id="id" value="{{$editaddr['oldaddr']['id']}}" >
            <input type="hidden"  name="formId" id="formId" value="{{$editaddr['oldaddr']['formId']}}" >
            {{csrf_field()}}
        <div class="title ">新增配送信息</div>
        <div class="addr-box">
            <label class=" "><span class="c-red">*</span>收件人：</label>
            <input type="text" class="addr-input" name="consignee"  id="consignee" value=" {{$editaddr['oldaddr']['consignee']}}" required maxlength="20"  autocomplete="off"/>
        </div>
        <div class="addr-box">
            <label class=" "><span class="c-red">*</span>手机号码：</label>
            <input type="text" class="addr-input" name="moble" id="moble"   value="{{$editaddr['oldaddr']['moble']}}" required maxlength="11" minlength="11"  autocomplete="off"/>
        </div>
            ...
            <div class="addr-box">
                <label class=" "><span class="c-red">*</span>所在地区：</label>
                    <select name="region1" id="region1"  class="select-box">
                        ...
                        @foreach($editaddr['province'] as $val)
                            <option value="{{$val['provinceID']}}" @if($val['provinceID'] ==$editaddr['oldaddr']['province'])  selected @endif> -- {{$val['province']}} --</option>
                        @endforeach
                    </select>
                <select name="region2" id="region2"  class="select-box">
                    <option value="0" > -- 城市 --</option>
                    @foreach($editaddr['city'] as $val)
                        <option value="{{$val['cityID']}}" @if($val['cityID'] ==$editaddr['oldaddr']['city'])  selected @endif> -- {{$val['city']}} --</option>
                    @endforeach
                </select>
        ...
        <div class="addr-box">
            ...
            <input type="text" class="addr-input" style=" width:350px;" name="district"  id="district" value=" {{$editaddr['oldaddr']['district']}}" required maxlength="20" autocomplete="off"/>
        </div>
            <div class="addr-box">
                <label class=" "></label>

                <input type="checkbox" name="isdefault"  id="isdefault" value="1" @if($editaddr['oldaddr']['is_default']==1)  checked  @endif /> 设为默认地址
            </div>
        <div class="addr-box">
    ...
    <script type="text/javascript">
        ...
        function saveconsignee() {
          ...
          if(province==0 ||city==0){
            ...
            $.get('/service/user',{act:'edit_addr', id:id,data:data},function(data){
                if(data.status == 0){
