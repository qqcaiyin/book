        ...
        <div   class="weui_cells weui_cells_checkbox">
            ...
            @foreach($cart_items as $cart_item)
                <label class="weui_cell weui_check_label" for="{{$cart_item->product->id}}">
                    <div class="weui_cell_hd" style="width:23px;">
                        <input type="checkbox" class="weui_check"  checked="checked" name="cart_item" id="{{$cart_item->product->id}}">
                        <i class="weui_icon_checked"></i>
                    </div>
                    <div class="weui_cell_bd weui_cell_primary">
                        <div style=" postion: relative;">
                            <img class="bk_preview" src="{{url($cart_item->product->preview)}}" class="m3_preview">
                            <div style="position: absolute;left:170px; right:0; top: 15px;">
                                <p style="font-weight: bold;">{{$cart_item->product->name}}</p>
                                <p class="bk_time" style="magin-top:15px; ">数量 <sapn class="bk_summary"> x{{$cart_item->count}}</sapn></p>
                                <p class="bk_time" >总计<sapn class="bk_price1">￥{{ $cart_item->product->price * $cart_item->count }}</sapn></p>
                            </div>
                        </div>
                    </div>
                </label>
            @endforeach
                <div class="bk_fix_bottom">laravel
                    ...
                    <div class="bk_half_area">
                        <button class="weui_btn weui_btn_default" onclick="_toDelete();" >删除</button>
                    </div>
                </div>
        </div>
    </div>
